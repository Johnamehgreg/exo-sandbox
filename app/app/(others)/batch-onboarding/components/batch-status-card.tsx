...
const getStatus = (status: string) => {
  switch (status) {
    ...
    case "in_progress":
      return (
        <span className="inline-flex gap-x-1 items-center px-2 h-7 rounded-[4px] border border-gray-300 bg-sky-100 text-darkblue font-medium capitalize">
          <IconCheckBox fill="#079abf" /> Ongoing
        </span>
      );
    case "failed":
      return (
    ...
    case "paused":
      return (
        <span className="inline-flex gap-x-2 items-center px-2 h-7 rounded-[4px] border border-gray-300 bg-orange-100 text-orange-600 font-medium capitalize">
          <WarningCircle weight="fill" className="" /> Paused
        </span>
      );
    default:
      return (
...
export const BatchStatusCard: React.FC<Props> = ({ status }) => {
